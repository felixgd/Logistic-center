...
export async function POST(req: NextRequest) {
  try {
    ...
    const token = signToken({ userId: user.id, actorId: parentActor.id, actorType: parentActor.type, csrfToken, diditStatus: parentActor.diditStatus, kycBlocked: parentActor.kycBlocked });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      return jsonError(400, "El correo, teléfono o código ya está registrado.");
    }
