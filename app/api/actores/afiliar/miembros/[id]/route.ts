...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!membership) return jsonError(404, "Voluntario no encontrado");
  ...
  return Response.json({ mensaje: "Voluntario actualizado" });
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return Response.json({ mensaje: "Voluntario eliminado" });
}
